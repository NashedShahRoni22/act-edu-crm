...
const incomeTypeOptions = ["Payables", "Income"];
...
export default function GeneralInvoice({ inline = true, onClose }) {
  ...
  const { data: servicesData, isLoading: isServicesLoading } = useQuery({
    queryKey: [
      `/services?partner_id=${partnerId}&with=partner,branches`,
      accessToken,
    ],
    queryFn: fetchWithToken,
  ...
  const selectedWorkflow = useMemo(
    () =>
      (selectedService?.available_workflows || []).find(
  ...
  // Bottom cards — split by income_type
  const totalPayables = items
    .filter((i) => i.income_type === "Payables")
    .reduce((s, i) => s + toNumber(i.amount), 0);
  const totalIncome = items
    .filter((i) => i.income_type === "Income")
    .reduce((s, i) => s + toNumber(i.amount), 0);
  ...
  const updateItem = (id, field, value) =>
    setItems((s) =>
      s.map((it) => {
        ...
        if (field === "tax_name") {
          updated.tax_rate = value === "GST (10%)" ? 10 : 0;
        }

        // Recalculate tax_amount whenever amount, tax_name, tax_rate, or is_tax_inclusive changes
        if (
          ["amount", "tax_name", "tax_rate", "is_tax_inclusive"].includes(field)
        ) {
          const amt = toNumber(updated.amount);
          const rate = updated.tax_name === "GST (10%)" ? 10 : 0;

          if (updated.is_tax_inclusive) {
            // Amount already includes tax — extract it
            updated.tax_amount = ((amt * rate) / (100 + rate)).toFixed(2);
          } else {
            updated.tax_amount = ((amt * rate) / 100).toFixed(2);
          }
        }

        // total = amount + tax (exclusive) or just amount (inclusive, tax is already inside)
        updated.total_amount = updated.is_tax_inclusive
          ? toNumber(updated.amount).toFixed(2)
          : (toNumber(updated.amount) + toNumber(updated.tax_amount)).toFixed(
              2,
            );
    ...
    <div className="max-w-[95vw] w-full max-h-[95vh] overflow-y-auto p-0">
      ...
      <div className="px-6 py-5 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-1.5">
              ...
              >
                <SelectTrigger className="w-full">
                  <SelectValue
                    placeholder={
                      partners.length ? "Select partner" : "No partners"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                disabled={!partnerId || isServicesLoading}
              >
                <SelectTrigger className="w-full">
                  <SelectValue
                    placeholder={
        ...
        {isClientMode ? (
          selectedApplication || selectedClient ? (
            ...
            <div className="text-sm text-gray-500">
              Select a client and application to view details.
            </div>
          )
        ) : selectedService ? (
          <div className="flex gap-4">
            <DetailCard
              ...
              rows={[
                ...
                {
                  label: "Branch ID",
                  value: selectedService.partner_branch_id
                    ? String(selectedService.partner_branch_id)
                    : null,
                },
              ]}
            />
          ...
          <div className="text-sm text-gray-500">
            Select a partner and product/service to view details.
          </div>
        )}
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <Select
                        ...
                        onValueChange={(v) =>
                          updateItem(item.id, "income_type", v)
                        }
                      >
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center justify-center">
                        <input
                          ...
                          className="w-4 h-4 accent-[#3B4CB8] cursor-pointer"
                        />
                      </div>
                    </td>
                    {/* Tax Amount */}
                    ...
                    <td className="px-3 py-2">
                      <span className="text-sm tabular-nums text-gray-700 px-2">
                        {toNumber(item.total_amount).toLocaleString("en-AU", {
        ...
        <div className="flex gap-6 items-start">
          {/* Left: Total Payables / Total Income cards */}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl border border-gray-200 p-4">
                ...
                <p className="text-3xl font-light text-gray-300 tabular-nums">
                  {totalPayables.toLocaleString("en-AU", {
                    minimumFractionDigits: 2,
                  })}
                </p>
              </div>
              <div className="rounded-xl border border-gray-200 p-4">
                ...
                <p
                  ...
                  style={{ color: totalIncome > 0 ? APP_BLUE : "#d1d5db" }}
                >
